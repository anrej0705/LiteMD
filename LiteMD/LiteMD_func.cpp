...
void LiteMD::closeEvent(QCloseEvent* ce)
{
	...
	if (fileChangedState)
	{
		...
		{
			//Проверяем получилось ли сохранить файл
			if (mde->slotSave())
			{
			...
			{
				//Отменяем событие если не удалось сохранить файл тем самым сохраняем данные от потери
				//давая пользователю ещё один шанс
				ce->ignore();
				return;
			}
		}
